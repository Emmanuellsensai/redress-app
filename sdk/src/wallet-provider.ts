/**
 * Bridges the Lace / 1am DApp Connector API to the `WalletProvider` /
 * `MidnightProvider` interfaces that `MidnightProviders` expects.
 *
 * Ported from the Anonymous Whispers SDK (Level 4 extraction) for use in
 * Redress. The bridge is contract-agnostic: only the JSDoc mentioning the
 * product name changes.
 *
 * These two sides do not speak the same language, which is the whole reason
 * this file exists:
 *
 ...
 * but a single object can satisfy both.
 */
 ...
 * `getEncryptionPublicKey` are *synchronous*, while the only way to obtain
 * those keys from the connector (`getShieldedAddresses`) is a promise. So the
 * keys are fetched once here and closed over, rather than fetched per call.
 *
...
export const createDAppConnectorWalletProvider = async (
  ...
  return {
    ...
    /**
     * Hands the proven-but-unbalanced transaction to the wallet, which pays
     * the fees, adds whatever inputs and outputs are needed to remove
     * imbalances, and binds it.
     *
